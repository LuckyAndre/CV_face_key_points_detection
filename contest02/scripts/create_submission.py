...
def main(args):
    ...
    for i, file_name in enumerate(tqdm.tqdm(files)):
        ...
        # 1. Segmentation.
        image, k, dw, dh = prepare_for_segmentation(image_src.astype(np.float) / 255., (256, 256)) # TODO! Учесть размер! Почему здесь не используются трансформации segmentation/transforms???
        ...
        # TODO: в каком формате возвращаются боксы???
        texts = []
        for box in boxes:
            box[[0, 2]] -= dw
            box[[1, 3]] -= dh
            ...
            crop = image_src[y1: y2, x1: x2, :] # TODO: вот здесь получился crop, который пойдет дальше!
